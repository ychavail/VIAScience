##################################################################
# Description: Defintion of all the functions/modules useful to plot statistics from the database ./data/data.csv. The modules are used in ./src/main_improved.py.
...
def read_data(file):
    ...
    - file is the .csv file in which the data is written
    """
...
def sort_data(df,gender):
    ...
    - df is the pandas dataframe containing data
...
def choose_bins(df,var,step):
    ...
    - df is the pandas dataframe containing data
    - var is the variable to assess, either 'age' or 'height'
    - step is the increment for bins intervals. Optimal option: 1. for age, 0.1 for height
    """
...
def plot_histogram(list,bin_1,bin_n,bins,side,label):
    ...
    - list is the np.array to asssess
    - bin_1 is the lower limit of bins for the histogram
    - bin_n is the upper limit of bins for the histogram
    - bins is the interval between two bar of the histogram
    - side defines where statistics are plotted: 'left' or 'right' of the interval
    - label is the label appearing in the legend. E.g. 'men' or 'women'
    """
...
def doublehistogram(df,var,step,res,path):
    ...
    - var is the variable to assess, either 'age' or 'height'
    - step is the increment for bins intervals. Optimal option: 1. for age, 0.1 for height
    - res is the resolution of the figure in dpi
    - path is the path were to store the output figure
    """
